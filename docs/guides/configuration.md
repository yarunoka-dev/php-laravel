...
- **The config is read lazily**, when the environment is first derived
  ...
  for the next scope, which a request or a queue job never notices —
  where one scope lives long (`tinker`, a test),
  `app()->forgetScopedInstances()` renews within the current scope,
  though it resets every scoped service, not only Yarunoka's.
